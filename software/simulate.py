...
def MNAR_self_mask_logistic(X: np.ndarray, p: float) -> np.ndarray:
    ...
    X_temp = X[:,:,0]
    # Pick coefficients so that W^Tx has unit variance (avoids shrinking)
    coeffs = pick_coeffs(X_temp, self_mask=True)
    # Pick the intercepts to have a desired amount of missing values
    intercepts = fit_intercepts(X_temp, coeffs, p, self_mask=True)

    ps = expit(X_temp * coeffs + intercepts)
...
def MNAR_mask_quantiles(
    ...
    if cut == "upper":
        quants = np.quantile(X_temp[:, idxs_na], 1 - q, dim=0)
        m = X_temp[:, idxs_na] >= quants
    elif cut == "lower":
        quants = np.quantile(X_temp[:, idxs_na], q, dim=0)
        m = X_temp[:, idxs_na] <= quants
    elif cut == "both":
        u_quants = np.quantile(X_temp[:, idxs_na], 1 - q, axis=0)
        l_quants = np.quantile(X_temp[:, idxs_na], q, axis=0)
        m = (X_temp[:, idxs_na] <= l_quants) | (X_temp[:, idxs_na] >= u_quants)
